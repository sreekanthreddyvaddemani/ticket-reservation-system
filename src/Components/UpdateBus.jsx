...
let handleSubmit=(e)=>{
    e.preventDefault()
    let data={name,from,to,cost_per_seat,bus_no,dop,seats,deptime,arrivaltime,travel_img}
axios.put(`http://localhost:8080/bus/${admin.id}/${bus.id}`,data)
.then(()=>{
...
useEffect(()=>{
    axios.get(`http://localhost:8080/bus/${index}`).then((response)=>{
        ...
        setBusNo(bus.bus_no)
        setDop(bus.dop) 
        setDepTime(bus.deptime)
        setSeats(bus.seats)
        setArrivalTime(bus.arrivaltime)
      setCost(bus.cost_per_seat)
      setTravel_img(bus.travel_img)
    })
    .catch(()=>{
