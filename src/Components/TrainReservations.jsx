...
const TrainReservations = () => {
let [content,setContent]=useState([])
let navigate=useNavigate()
let {index}=useParams()
useEffect(()=>{
    axios.get(`http://localhost:8080/train/${index}`).then((res)=>{
        setContent(res.data.data.traintickets)
    }).catch(()=>{
        alert("No data")
    })
  },[])
 let onBack=()=>{
    navigate('/adminhomepage/trainlist')
  }
  return (
    <div>
...
export default TrainReservations
