...
let filter=JSON.parse(localStorage.getItem('filter'))
  ...
  localStorage.setItem("bus",JSON.stringify(value))
    ...
    let back=()=>{
     ...
     useEffect(()=>{
      axios.get(`http://localhost:8080/bus/filter?from=${filter.from}&to=${filter.to}&dop=${filter.dop}`)
  .then((res)=>{
  ...
  return (
    <div>
      ...
      <div className="spantab">
            ...
            <Button id="costbutton" variant="contained" color='error' placeholder='MM/DD/YYYY'  disableElevation focused onClick={()=>{bookBus(x)}}>SHOW SEATS</Button > 
